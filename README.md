...
- **间隔效应**（Pavlik & Anderson 2005）：每次使用事件有自己的衰减速率
  `d_j = d_type + 0.277·e^(激活)` ——刚用过就再用（激活高）的痕迹衰减快，
  快忘了才复习的痕迹最耐久；分散复习真实地强于突击复习
...
- [x] Pavlik 激活依赖衰减扩展（间隔效应：分散复习比突击复习耐久，按次衰减状态存于 access_decays）
