...
让 Codex (OpenAI agent) 能够接入 Hub，与其他 agent 交流。
...
使用方式：
    # 作为独立进程运行
    python -m adapters.codex_adapter --name "Codex"
...
class CodexAdapter(BaseAdapter):
    ...
    def __init__(self, name: str = "Codex", hub_url: str = None):
        """
        初始化 Codex 适配器
        ...
        Args:
            ...
            hub_url: hub 服务器地址
        """
        super().__init__(name, agent_type="codex", hub_url=hub_url)
    ...
    def _write_task(self, task: Dict[str, Any]):
        """写入任务文件"""
        try:
            task_file = "/tmp/codex_hub_task.json"
...
def main(argv: List[str] = None) -> int:
    ...
    parser = argparse.ArgumentParser(
        prog="codex-adapter",
        description="Codex adapter for Communication Hub.",
    )
